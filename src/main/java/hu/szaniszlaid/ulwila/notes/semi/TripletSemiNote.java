...
public class TripletSemiNote extends TripletNote {
	...
	public void drawNote(Graphics2D g) {
		...
		Arc2D left = new Arc2D.Double(0, 0, QUARTER_NOTE_WIDTH, QUARTER_NOTE_HEIGHT, 90, 180, Arc2D.CHORD);
		g.fill(left);
		g.setColor(getLeftColor());
		Arc2D right = new Arc2D.Double(0, 0, QUARTER_NOTE_WIDTH, QUARTER_NOTE_HEIGHT, 110, 140, Arc2D.CHORD);
		g.fill(right);
		g.setColor(Color.BLACK);
		Arc2D.Double border = new Arc2D.Double(0, 0, QUARTER_NOTE_WIDTH, QUARTER_NOTE_HEIGHT, 90, 180, Arc2D.CHORD);
		g.draw(border);
	}

	@Override
	public List<Shape> getOctaveShapes() {
		int centerX = (QUARTER_NOTE_WIDTH / 2 - QUARTER_NOTE_WIDTH / 10);
		int centerY = (QUARTER_NOTE_HEIGHT / 2 - QUARTER_NOTE_HEIGHT / 10);
